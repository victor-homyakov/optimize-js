...
const argv = yargs
  ...
  .option('handle-function-declarations', {
    alias: 'handleFunctionDeclarations',
    choices: ['none', 'safe', 'unsafe'],
    describe: 'Specify how to handle FunctionDeclaration nodes: "none" skips them completely; "unsafe" wraps all declarations; "safe" wraps only declarations that are not used before declaration and thus preserves hoisting (that otherwise can result in broken code, when the function is called before being assigned to variable).'
  })
  .help('h')
  .alias('h', 'help')
  .argv
...
inStream.pipe(concat(function (buf) {
  ...
  const opts = {
    ...
    handleFunctionDeclarations: argv['handle-function-declarations']
  }
  try {
