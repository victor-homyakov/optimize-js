...
function optimizeJs (jsString, opts) {
  ...
  let magicString = new MagicString(jsString)
  ...
  walk(ast, {
    enter: function (node, parent) {
      ...
      function isElementOfArray (node) {
        const parent = getParent(node)
        return isArrayExpression(parent) &&
      ...
      function isValueOfObjectLiteralWithNumericName (node) {
        const parent = node ? getParent(node) : undefined
        return parent &&
          parent.type === 'Property' &&
          parent.key &&
          parent.key.type === 'Literal' &&
          parent.key.raw &&
          isNumeric(parent.key.raw) &&
          parent.value === node &&
          getParent(parent) &&
          getParent(parent).type === 'ObjectExpression'
      }
      ...
      function isProbablyWebpackModule (node) {
        return isElementOfArray(node) && isArgumentToFunctionCall(getParent(node))
      }

      function isProbablyBrowserifyModule (node) {
        return isElementOfArray(node) &&
          isValueOfObjectLiteralWithNumericName(getParent(node)) &&
          isArgumentToFunctionCall(getParent(getParent(getParent(node)))) &&
          isIIFECall(getParent(getParent(getParent(getParent(node)))))
      }
