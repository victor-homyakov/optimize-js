  ...
  walk(ast, {
    enter: function (node, parent) {
      ...
      function isPaddedArgument (node) {
        const parentArray = node.parent().arguments ? node.parent().arguments : node.parent().elements
      ...
      if (node.type === 'FunctionExpression' || node.type === 'FunctionDeclaration') {
        ...
        if (isArgumentToFunctionCall(node) ||
          ...
          if (!isPaddedArgument(node)) { // don't double-pad
            magicString = magicString.appendLeft(node.start, '(')
              .appendLeft(node.end, ')')
          }
        } else if (isIIFE(node)) {
          // this function is getting immediately invoked, e.g. function(){}()
          if (preChar !== '(') {
            magicString.appendLeft(node.start, '(')
              .appendLeft(node.end, ')')
          }
        } else if (node.type === 'FunctionDeclaration') {
          // function x() {} => var x=(function x() {});
          magicString.appendLeft(node.start, `var ${node.id.name}=(`).appendLeft(node.end, ');')
        }
      }
    }
  })
...
module.exports = { run: optimizeJs, optimizeJsRollupPlugin }
